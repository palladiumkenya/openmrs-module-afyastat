...
/**
 */
public class PatientLookUpUtils {
	
	private PatientLookUpUtils() {
	}
	...
	public static Patient findSavedPatient(Patient candidatePatient, boolean searchRegistrationData) {
		...
		if (savedPatient == null
		        && (candidatePatient.getPatientIdentifier() != null && StringUtils.isNotEmpty(candidatePatient
		                .getPatientIdentifier().getIdentifier()))) {
			List<Patient> patients = Context.getPatientService().getPatients(
			    candidatePatient.getPatientIdentifier().getIdentifier());
			savedPatient = PatientLookUpUtils.findSimilarPatientByNameAndGender(patients, candidatePatient);
		}
		...
		if (savedPatient == null && candidatePatient.getPersonName() != null
			...
			List<Patient> patients = Context.getPatientService().getPatients(candidatePatient.getPersonName().getFullName());
			savedPatient = PatientLookUpUtils.findSimilarPatientByNameAndGender(patients, candidatePatient);
		}

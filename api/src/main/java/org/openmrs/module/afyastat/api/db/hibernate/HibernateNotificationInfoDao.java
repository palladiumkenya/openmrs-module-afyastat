...
public class HibernateNotificationInfoDao extends HibernateInfoDao<NotificationInfo> implements NotificationInfoDao {
	 ...
	 * Default constructor.
	 */
	protected HibernateNotificationInfoDao() {
	...
	public List<NotificationInfo> getNotificationsByReceiver(final Person person, final String search,
	...
	@Override
	@SuppressWarnings("unchecked")
	public List<NotificationInfo> getNotificationsBySender(final Person person, final String search,
	        final Integer pageNumber, final Integer pageSize, final String status, final Date syncDate) {
		Criteria criteria = getSessionFactory().getCurrentSession().createCriteria(mappedClass);
		if (StringUtils.isNotEmpty(search)) {
			Disjunction disjunction = Restrictions.disjunction();
	...
	@Override
	public List<NotificationInfo> getNotificationsByRole(final Role role, final String search, final Integer pageNumber,
